...
- 客户端和服务器开始新的重建连接的过程
### TLS1.3
上面的协议都是 TLS1.2 以及之前的版本, 我们发现正常的 TLS1.2 的连接需要经过 2RTT,即使是被session缓存过的 TLS 连接也需要经过 1RTT 的时间

而到了TLS1.3,正常的连接只需要1-RTT, 而缓存过的连接可以达到0-RTT

1. 1-RTT握手: 将客户端发送ECDH临时公钥的过程提前到ClientHello, 同时删除了ChangeCipherSpec协议简化握手过程
- 客户端发送ClientHello消息,该消息主要包括客户端支持的协议版本、DH密钥交换参数列表KeyShare
- 服务端回复serverHello, 包含选定的加密套件;发送证书给客户端;使用证书对应的私钥对握手消息签名,将发送结果发送给客户端;选用客户端提供的参数生成ECDH零时公钥,结合选定的DH参数计算出用于加密HTTP消息的共享密钥;服务端神吐槽的临时公钥通过KeyShare消息发送给客户端
- 客户端接收到keyShare消息之后,使用证书公钥进行签名验证,获取服务端的ECDH临时公钥,生成绘画所需要的共享密钥
- 之后双方使用生成的共享密钥对消息加密传输,保证消息安全

2. 0-RTT握手: 对于访问过的网站可以在第一次交互时就将加密数据发送给服务器
- 客户端和服务端通过TLS session 复用或外部输入的方式共享PSK,这种情况下允许客户端在第一次交互的ClientHello消息中包含应用数据,该数据用PSK加密。