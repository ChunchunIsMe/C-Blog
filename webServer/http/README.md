...
- 并且QUIC安全层同样适用TLS1.3协议(且QUIC必须通过TLS1.3来建立连接)
### http3.0特点
- 基于UDP
- 可靠性
- 数据流: 连接在两个端点之间经过类似TCP连接的方式协商建立.QUIC连接基于UDP端口和IP地址建立,一旦连接建立,连接通过连接ID关联。在已经建立的连接上,双方均可以建立传输给对方的数据流。单一数据流的传输是可靠、有序的,但不同的数据流间可能无序传送
- 有序交付: 单个数据流之间可以有序交付,但是多个数据流之间可能乱序。在QUIC中丢包只会影响该包所处的流,不会拥塞整个连接
- 快速握手: QUIC提供0-RTT和1-RTT的连接建立,这意味着QUIC在最佳情况下不需要任何往返时间就可以建立连接,其中更快的0-RTT在两个主机之间建立过连接并且缓存了连接的secret时可以使用,QUIC允许客户端在0-RTT的情况下直接捎带数据。这使得客户端能尽早向对方传送数据,当然也使得服务器更快发回数据响应
- TLS1.3(安全握手协议)
- 传输层与应用层协议(http3.0)
- QUIC之上的HTTP协议: 使用不同的头部压缩算法(效果和http2类似,用新的算法的原因是因为http2的算法需要有序交付,http3有可能乱序)
### QUIC原理
- 连接
   - 连接ID(Connection ID): 每个连接过程都有一组连接标识符,称为连接ID,该ID用以识别该连接。每个端点各自选择连接ID。每个端点选择对方使用的连接ID(这样可以做到连接迁移,就算你IP和网络接口改变你也能够得到连接保持)
   - 端口号: QUIC基于UDP连接,所以使用16比特的UDP端口号字段来区分传入不同的连接
   - 版本协商: 客户端的QUIC连接请求会告诉服务器所希望使用的QUIC协议版本
- 必须使用TLS的连接
- 数据流: 有两种基本的数据流类型从发起者到对等端的单向数据流和双向均可发出数据的双向数据流
   - 流量控制: 每一个数据流都有自己的流量控制,端点可以通过此实现内存控制和反压。数据流的创建本身也有流量控制,连接双方可以声明最多愿意创建几个流ID
   - 流标识符: 数据流通过无符号的62比特整数标识,也称为流ID。流ID最后2位比特用于识别流的类型(单(1)/双向(0))和流的发起者(客户端双数/服务器单数)
   - 流并发: QUIC允许任意数量的流并发。端点通过闲置最大流ID来控制并发的传入流数量
   - 收发数据: 端点使用流来收发数据,这是流的最终用途。QUIC数据流是有序的字节流抽象。但是,不同流之间是无序的。
   - 流优先度: 可以和HTTP2.0一样设置流的优先度
- 0-RTT: 先前已经练过一个服务器的客户端可能缓存连接的参数,并在之后与服务器建立一个无需等待握手完成的就可以立即传输信息的0-RTT连接。