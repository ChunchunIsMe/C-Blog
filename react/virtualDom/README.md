...
那么传统的 diff 算法是循环现有树每一个节点,让这个节点和生成树的每个节点进行对比。在查找完差异之后还要机选最小转换方式算法的复杂度就达到了o(n^3)
![传统diff](./picture/1.webp)
而 react 的 diff 算法是循环现有树的每一个节点,让这个节点和生成树的同层级进行对比。这种算法的时间复杂度是o(n)
![react-diff](./picture/2.webp)